...
module.exports = {
    ...
    create_patDets: async(req, res) => {
        ...
        const {age, gender, bloodgroup, allergies, occur_cond, medication, emergency_no} = req.body;
            ...
            const newPatDets = new patdts({patient: req.body.userId, age, gender, bloodgroup, allergies, occur_cond, medication, emergency_no})
    ...
    try
    {
        ...
        const patdets = await patdts.find({})
        .select('-patient.__v -patient._id -patient.patient_email -patient.password -patient.patient_phone_no')
        .sort('-createdAt');
...
patSearch: async(req, res, next) => {
    ...
    try {
       ...
       //const patd = await pats.findOne({'patient.UID': req.body.UID})
       
   
           const patdet = await patdts.findOne({'patient.UID': req.body.UID}).select('-patient.__v -patient._id -patient.patient_email -patient.password -patient.patient_phone_no')
