...
const Patient = require("../models/patientModel").users;
      ...
      switch (method) {
        ...
        case 'docReg': {
          ...
          return [
            ...
            body('doctor_phone_no', 'doctor mobile number not entered').notEmpty().isInt().withMessage('doctor mobile number invalid').isLength({min: 10, max: 10}).withMessage('doctor mobile number should be 10 digits').trim().escape(),
        ...
        case 'orgReg': {
          ...
          return [
            ...
            body('org_phone_no', 'organization contact number not entered').notEmpty().isInt().withMessage('organization contact number invalid').trim().escape(),
            ...
            body('reddrsl_no', 'organization reddressal number not entered').notEmpty().isInt().withMessage('organization redressal number invalid').trim().escape()
          ]
        }
    ...
    patientReg: async (req, res) => {
      ...
      login: async (req, res) => {
        try {
          ...
          const token = jwt.sign({userId: user}, process.env.JWT_SECRET, {expiresIn: '12h'});
          res.json({
            status: 'success',
            message: 'user found !!',
            token: token,
